...
#define ADE_SCL_HCOEF(x, m, n)    (0x3100 + (x) * 0x800 + 12 * (m) + 4 * (n))
#define ADE_SCL_VCOEF(x, i, j)    (0x340C + (x) * 0x800 + 12 * (i) + 4 * (j))
/* ctran regs */
...
/* bits mask */
#define BITS2_MSK	(0x3)
#define BITS3_MSK	(0x7)
#define BITS4_MSK	(0xF)
...
static inline void set_LDI_CTRL_ldi_en(u8 *base, u32 nVal)
{
...
static inline void set_LDI_CTRL_disp_mode(u8 *base, u32 nVal)
{
	volatile union U_LDI_CTRL ldi_ctrl;
	u8 *addr = base + LDI_CTRL;
...
static inline void set_LDI_CTRL_bpp(u8 *base, u32 nVal)
{
	volatile union U_LDI_CTRL ldi_ctrl;
	u8 *addr = base + LDI_CTRL;
...
static inline void set_LDI_CTRL_corlorbar_width(u8 *base, u32 nVal)
{
	volatile union U_LDI_CTRL ldi_ctrl;
	u8 *addr = base + LDI_CTRL;
...
static inline void set_LDI_CTRL_bgr(u8 *base, u32 nVal)
{
	volatile union U_LDI_CTRL ldi_ctrl;
	u8 *addr = base + LDI_CTRL;
...
static inline void set_LDI_WORK_MODE_work_mode(u8 *base, u32 nVal)
{
...
static inline void set_LDI_WORK_MODE_colorbar_en(u8 *base, u32 nVal)
{
	volatile union U_LDI_WORK_MODE ldi_work_mode;
	u8 *addr = base + LDI_WORK_MODE;
