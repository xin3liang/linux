...
static void i2c_clk_domain_enable(struct dw_i2c_dev *dev, unsigned int
enable)
{
       unsigned int ret;
       int timeout = 10;
       ...
       if (enable)
               writel(BIT(dev->reset_bit), sctrl_base +
					   dev->reset_enable_off);
       else
               writel(BIT(dev->reset_bit), sctrl_base +
					   dev->reset_disable_off);
...
static int dw_i2c_probe(struct platform_device *pdev)
{
	...
	if(!sctrl_base) {
		node = of_find_compatible_node(NULL,
						NULL,"hisilicon,sysctrl");
		if (node)
			sctrl_base = of_iomap(node, 0);
	}

	of_property_read_u32_array(pdev->dev.of_node, "reset-controller-reg",
					&data[0], 4);
